  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
      <div className="p-4 bg-white rounded-lg shadow">
        <div className="text-sm text-slate-500">Today's Spend</div>
        <div className="text-2xl font-semibold">{formatCurrency(Number(todaySpent || 0), currency)}</div>
      </div>

      <div className="p-4 bg-white rounded-lg shadow">
        <div className="text-sm text-slate-500">This Month</div>
        <div className="text-2xl font-semibold">{formatCurrency(Number(monthSpent || 0), currency)}</div>
      </div>

      <div className="p-4 bg-white rounded-lg shadow">
        <div className="text-sm text-slate-500">Budget</div>
        <div className="text-2xl font-semibold">{formatCurrency(Number(budget || 0), currency)}</div>
        <div className="text-xs text-slate-400">{Math.round(percentUsed || 0)}% used — {formatCurrency(Number(remaining || 0), currency)} left</div>
        <div className="mt-3">
